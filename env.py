...
class Environment:
    def __init__(
        ...
        :param map_file: Path to the map text file.
        :param move_cost: Cost incurred when a robot moves (LRUD).
        :param delivery_reward: Reward for delivering a package on time."""
    ...
    def reset(self):
        ...
        for i in range(self.n_packages):
            ...
            self.packages.append(
                Package(start, start_time, target, deadline, package_id)
            )
        ...
        while True:
            ...
            for i in range(len(self.robots)):
                ...
                if new_pos not in old_pos:
                    ...
                    if (j != i) and (
                        computed_moved[j] == 0
                    ):  # We must wait for the conflict resolve
                        continue
            ...
            if pkg_act == "1":
                if robot.carrying == 0:
                    ...
                    for j in range(len(self.packages)):
                        if (
                            self.packages[j].status == "waiting"
                            and self.packages[j].start == robot.position
                            and self.packages[j].start_time <= self.t
                        ):
...
if __name__ == "__main__":
    ...
    agents.init_agents(
        state
    )  # You have a change to init states which can be used or not. Depend on your choice
