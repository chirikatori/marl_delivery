...
def run_bfs(map, start, goal):
    ...
    while queue:
        ...
        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            ...
            if (
                next_pos[0] < 0
                or next_pos[0] >= n_rows
                or next_pos[1] < 0
                or next_pos[1] >= n_cols
            ):
                continue
...
class GreedyAgents:
    ...
    def init_agents(self, state):
        ...
        self.packages += [
            (p[0], p[1] - 1, p[2] - 1, p[3] - 1, p[4] - 1, p[5])
    ...
    def update_move_to_target(self, robot_id, target_package_id, phase="start"):
        ...
        if phase == "start":
            distance = abs(
                self.packages[target_package_id][1] - self.robots[robot_id][0]
            ) + abs(self.packages[target_package_id][2] - self.robots[robot_id][1])
        else:
            # Switch to the distance to target (3, 4) if phase == 'target'
            distance = abs(
                self.packages[target_package_id][3] - self.robots[robot_id][0]
        ...
        if distance >= 1:
            ...
            move, distance = run_bfs(
                self.map, (self.robots[i][0], self.robots[i][1]), target_p
            )
        ...
        for i in range(self.n_robots):
            ...
            if self.robots_target[i] != "free":
                ...
                if self.robots[i][2] != 0:
                    ...
                    move, action = self.update_move_to_target(
                        i, closest_package_id - 1, "target"
                    )
                ...
                for j in range(len(self.packages)):
                    ...
                    pkg = self.packages[j]
                    d = abs(pkg[1] - self.robots[i][0]) + abs(
                        pkg[2] - self.robots[i][1]
                    )
